...
"""This model implements the permutation flow shop problem with OR-Tools.

In the permutation flow shop problem (PFSP), a set of jobs has to be processed
on a set of machines. Each job must be processed by each machine in sequence
and all jobs have to be processed in the same order on every machine.
...
def permutation_flow_shop(
    ...
    for idx1 in range(num_jobs):
        circuit.append((0, idx1 + 1, m.new_bool_var("start")))
        circuit.append((idx1 + 1, 0, m.new_bool_var("end")))
    ...
    for idx1, idx2 in product(range(num_jobs), repeat=2):
        if idx1 != idx2:
            ...
            lits[idx1, idx2] = lit
            circuit.append((idx1 + 1, idx2 + 1, lit))
...
def parse_args():
    ...
    parser.add_argument("--time-limit", type=float, default=float("inf"), help=msg)
...
if __name__ == "__main__":
    ...
    permutation_flow_shop(np.array(PROCESSING_TIMES), args.time_limit, args.log)
